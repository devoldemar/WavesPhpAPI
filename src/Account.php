...
class Account {
	...
	public function __construct($seed = '') {
		if ($seed) {
			if ( count(explode(' ', self::from58($seed))) < 15 )
		...
		$keyPair = $this->keypair25519($this->buildSeedHash());
		$this->publicKey = self::to58($keyPair['publicKey']);
		$this->privateKey = self::to58($keyPair['privateKey']);
	...
	public function buildSeedHash($nonce = 0) {
		...
		$noncedSecretPhrase = pack('N', self::INITIAL_NONCE) . self::from58($this->seed);
		
		$seedHash = self::chainedHash($noncedSecretPhrase);
		$sha256Hash = hash('sha256', $seedHash, true);
		return $sha256Hash;
	}
	
	public function buildAddress() {
		$publicKey = self::from58($this->publicKey);
		$publicKeyHash = self::chainedHash($publicKey);

		$unhashedAddress = pack('C', self::ADDRESS_VERSION) . pack('C', (ord(self::NETWORK_CODE) & 0xFF)) . substr($publicKeyHash, 0, 20);
		$addressHash = substr( self::chainedHash($unhashedAddress), 0, 4 );

		return self::to58($unhashedAddress . $addressHash);
	}
